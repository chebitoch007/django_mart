...
import { setSubmitButtonState, startProcessingAnimation, stopProcessingAnimation, showPaymentError, showPaymentStatus } from '../ui/ui.js';
    ...
    try {
        ...
        if (data.success && data.checkout_request_id) {
            ...
            try {
                ...
                if (hidden)
                    hidden.value = data.checkout_request_id;
            }
            catch (e) { /* ignore */ }
...
function pollMpesaPaymentStatus(checkoutRequestId, paymentSystem) {
    ...
    const poller = setInterval(async () => {
        ...
        if (attempts > maxAttempts) {
            ...
            stopProcessingAnimation(elements.processingModal);
            setSubmitButtonState(false, elements.paymentSubmitButton);
            showPaymentError('Payment timed out. Please check your phone to complete the M-Pesa payment, or try again.', elements.paymentErrors);
            return;
        }
        try {
            ...
            if (json.status === 'success' || json.status === 'completed') {
                ...
                stopProcessingAnimation(elements.processingModal);
                setSubmitButtonState(false, elements.paymentSubmitButton);
                showPaymentError(json.message || 'Payment failed or was cancelled. Please try again.', elements.paymentErrors);
            }
            else {

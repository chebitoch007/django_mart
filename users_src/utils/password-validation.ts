// users_src/utils/password-validation.ts
...
export function initializePasswordValidation(): void {
    ...
    passwordFields.forEach((field) => {
        const passwordInput = field as HTMLInputElement;

        // Only attach to new password fields, not old/current password
        if (passwordInput.id.includes('old_password') || passwordInput.id.includes('current')) {
            return;
        }
        ...
        passwordInput.addEventListener('input', (e) => {
            ...
            // Update strength bars if they exist
            if (bars.lengthBar) {
                updateStrengthBars(password, bars);
            }
...
declare global {
    interface Window {
        PasswordValidator: {
            ...
            getStrengthText: typeof getStrengthText;
            updateStrengthBars: typeof updateStrengthBars;
            updateRequirementsList: typeof updateRequirementsList;
        };
    }
}
